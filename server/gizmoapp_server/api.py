...
COACH_COMPONENTS = {
    "rear-wheel": "repairing a flat tire and inspecting rear wheel tubes, tires, and rims",
    "front-wheel": "inspecting front wheel tires, axles, rims, and wheel safety",
    "brakes": "cleaning and adjusting bicycle brakes, brake pads, cables, and rotors",
    "chain": "cleaning, degreasing, lubricating, and inspecting a bicycle chain",
    "gears": "adjusting bicycle shifting, derailleurs, limit screws, and cable tension",
    "saddle": "adjusting bicycle saddle height, angle, and clamp safety",
    "handlebar": "aligning bicycle handlebars, stems, controls, and steering",
    "pedals": "inspecting and tightening bicycle pedals, cranks, and pedal threads",
}
...
def register_api_routes(app: Flask) -> None:
    ...
    @app.post(scoped_path(prefix, "api/coach"))
    def coach():
        image_data = None
        image_type = None
        if request.is_json:
            ...
            if image and image.filename:
                ...
                if image_type not in COACH_IMAGE_TYPES:
                    return _error_response("image must be a JPEG, PNG, WebP, or GIF", 400)
                image_data = image.read(MAX_COACH_IMAGE_BYTES + 1)
                if len(image_data) > MAX_COACH_IMAGE_BYTES:
                    return _error_response("image must be at most 8 MB", 413)
                if not image_data:
                    return _error_response("image must not be empty", 400)
        else:
            return _error_response("Content-Type must be application/json or multipart/form-data", 415)
        component = payload.get("component")
        question = payload.get("question")
        if component not in COACH_COMPONENTS:
            return _error_response("component must be a supported bike repair component", 400)
        if not isinstance(question, str) or not question.strip():
            return _error_response("question must be non-empty text", 400)
        question = question.strip()
        if len(question) > MAX_COACH_QUESTION_LENGTH:
        ...
        if step_instruction and (not isinstance(step_instruction, str) or len(step_instruction) > 500):
            return _error_response("step_instruction must be at most 500 characters", 400)
        user_content: str | list[dict[str, Any]] = question
        if image_data and image_type:
            encoded_image = base64.b64encode(image_data).decode("ascii")
            user_content = [
                {"type": "text", "text": question},
                {"type": "image_url", "image_url": {"url": f"data:{image_type};base64,{encoded_image}"}},
            ]
        messages = [
            {"role": "system", "content": (
        ...
        try:
            answer = chat(messages, max_tokens=500)
