...
def _validate_messages(messages: Sequence[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for message in messages:
        if not isinstance(message, dict):
            raise CourseLLMError("each message must be an object with a role and valid content.")
        role = message.get("role")
        content = message.get("content")
        if role not in ALLOWED_ROLES or not _valid_content(content):
            raise CourseLLMError(
                "each message must have a system, user, or assistant role and non-empty text or image content."
            )
...
def _valid_content(content: Any) -> bool:
    ...
    for part in content:
        if not isinstance(part, dict) or part.get("type") not in {"text", "image_url"}:
            return False
        if part["type"] == "text" and (not isinstance(part.get("text"), str) or not part["text"].strip()):
            return False
        if part["type"] == "image_url":
            image_url = part.get("image_url")
            if not isinstance(image_url, dict) or not isinstance(image_url.get("url"), str) or not image_url["url"].startswith("data:image/"):
                return False
    return True
