...
Spoke & Wrench is an interactive bicycle repair companion for everyday riders. Select a part on the commuter-bike diagram, open a focused repair guide, and follow practical steps without needing to know mechanic jargon. The app is a sample project developed for [UBC's AI 100: Introduction to Artificial Intelligence](https://www.cs.ubc.ca/~kevinlb/teaching/ai100/).

## What It Does

- Interactive bike diagram with guides for the front wheel, rear wheel, brakes, chain, gears, saddle, handlebar, and pedals
- Search and quick picks for common problems such as flat tires, squeaky brakes, slipping chains, and uncomfortable seats
- Short repair guides with time estimates, difficulty, required tools, safety tips, and step-by-step instructions
- “Ask the mechanic” AI coaching for a selected component or individual repair step
- Optional photo upload so the mechanic can answer questions about what is visible on the bike
- Bookmark guides and individual steps in a “Saved for later” list
- Print-friendly instructions for the active guide
- Light and dark display modes with a responsive layout for phones and desktop screens

The app does not show a repair preview until a bike component or search result is selected. It also avoids progress tracking: repair steps remain available as reference material rather than becoming a checklist.

## Technology

- Python `Flask` backend with prefix-aware routes
- Build-free HTML, CSS, and JavaScript frontend
- SQLite for the starter application's persistence layer
- Course AI model integration through the bundled `llm.py` helper
- Optional image questions sent as multipart form data to the Flask `/api/coach` route

The public experience uses the graphical shell in `server/gizmoapp_server/templates/index.html`. The text shell and shared GizmoApp infrastructure remain in the repository for compatibility with the hosted workspace.

## Why No Frontend Build Step

The project intentionally avoids Node and a frontend bundler. The bike diagram and repair guides are delivered as ordinary HTML, CSS, and JavaScript, which keeps the hosted preview and future course edits simple. The frontend is split into small modules while preserving the path-prefix-aware configuration supplied by Flask.

## Repository Layout

- `server/` contains the Flask app, SQLite wiring, and HTML/CSS/JS assets
- `server/gizmoapp_server/templates/index.html` contains the Spoke & Wrench graphical shell
- `server/gizmoapp_server/static/app/main.js` contains the component guides, search, bookmarks, print action, theme toggle, and mechanic interaction
- `server/gizmoapp_server/static/app/styles.css` contains the responsive light/dark visual design and print rules
...
The bike-repair UI uses these prefix-aware routes:

- `POST /api/coach` answers a question about the selected bike component or repair step; it accepts JSON or multipart form data with an optional JPEG, PNG, WebP, or GIF photo
- `GET /api/bootstrap` returns app metadata, health details, and available shell summaries
- `GET /api/capabilities` returns capability status
...
Bookmarks are intentionally kept in the browser UI for this sample project, so they do not require an account or server-side profile. If `GIZMOAPP_URL_PREFIX` is set, routes live under that prefix; for example, `/demo-app/api/coach`.
